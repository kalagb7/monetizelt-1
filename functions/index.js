...
/* ============================= STRIPE WEBHOOK (CREATES purchases + access code) ============================= */
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (!appSessionId || !productId || !sellerUid || !buyerEmail) {
                console.error("Fulfillment missing metadata:", session.id);
                res.json({ received: true });
                return;
            }
            ...
            const product = productDoc.data();
            if (product.status !== PRODUCT_STATUS.ACTIVE) {
                console.warn("Product not active:", productId, product.status);
                res.json({ received: true });
                return;
            }
            ...
            try {
                ...
                console.error("Could not retrieve exact Stripe fee:", e?.message);
            }
            if (stripeFee === null) stripeFee = productPrice * STRIPE_FEE_RATE_FALLBACK + STRIPE_FEE_FIXED_FALLBACK;
            ...
            const orderDeviceInfo = ps.deviceInfo || { browser: "Unknown", os: "Unknown", device: "Unknown", userAgent: "" };
            const deviceFingerprint = String(ps.deviceFingerprint || "").trim() || null;
            const deviceFingerprintHash = ps.deviceFingerprintHash || (deviceFingerprint ? sha256Hex(deviceFingerprint) : null);
            ...
            await db.runTransaction(async (tx) => {
                ...
                if (!freshPS.exists) throw createHttpError(404, "not_found", "Payment session not found.");
                if (freshPS.data()?.completed) return;
                ...
                if (deviceFingerprintHash) {
                    tx.set(
                        db.collection("deviceToBuyer").doc(String(deviceFingerprintHash)),
                        {
                            deviceFingerprintHash,
                            email: buyerEmail,
                            emailHash: buyerEmailHash,
                            verifiedAt: nowServerTimestamp(),
                            source: "purchase",
                            lastSeenAt: nowServerTimestamp(),
                            updatedAt: nowServerTimestamp(),
                        },
            ...
            try {
                ...
                if (sellerEmail && isEmail(sellerEmail)) {
                    ...
                    await sendEmail("seller_sale_notification", {
                        ...
                        preheader: `Earnings added: ${sellerAmount.toFixed(2)} USD`,
                        bodyHtml: `
                          <p>Good news — your product was purchased.</p>
                          <div style="margin:12px 0;">
                            ${kvTable([
                            ["Product", `<span style="font-weight:800;">${productTitle}</span>`],
...
/* ============================= BUYER LIBRARY (DEVICE FIRST, ELSE EMAIL+CODE) ============================= */

exports.getBuyerLibrary = onRequest({ invoker: "public", secrets: [appBaseUrl] }, async (req, res) => {
    corsMiddleware(req, res, async (req2, res2) => {
        ...
        try {
            ...
            const userAgent = String(req2.get("user-agent") || req2.body?.userAgent || "");
            const deviceFingerprint = String(req2.body?.deviceFingerprint || getDeviceFingerprintFromReq(req2) || "").trim();

            const fpHash = deviceFingerprint ? sha256Hex(deviceFingerprint) : null;

            let emailNorm = normalizeEmail(email);
            let emailHash = emailNorm ? sha256Hex(emailNorm) : null;

            // Step 1: try device
            if (fpHash) {
                const mapSnap = await db.collection("deviceToBuyer").doc(fpHash).get();
                if (mapSnap.exists) {
                    const m = mapSnap.data() || {};
                    if (m.emailHash) {
                        emailHash = String(m.emailHash);
                        emailNorm = String(m.email || "");
                    }
                    await mapSnap.ref.set({ lastSeenAt: nowServerTimestamp(), updatedAt: nowServerTimestamp() }, { merge: true }).catch(() => null);
                }
            }

            // Step 2: if not found, require email+code
            if (!emailHash) {
                if (!emailNorm || !isEmail(emailNorm)) throw createHttpError(400, "invalid_argument", "Email is required.");
                if (!code || String(code).trim().length < 6) throw createHttpError(400, "invalid_argument", "Access code is required.");

                emailHash = sha256Hex(emailNorm);
                ...
                if (fpHash) {
                    await db.collection("deviceToBuyer").doc(fpHash).set(
                        {
                            deviceFingerprintHash: fpHash,
                            email: emailNorm,
                            emailHash,
                ...
                await codeSnap.ref.set({ lastUsedAt: nowServerTimestamp(), lastUsedUA: extractDeviceInfo(userAgent) }, { merge: true });
            }

            const purchasesSnap = await db.collection("purchases").where("emailHash", "==", emailHash).orderBy("createdAt", "desc").limit(100).get();
            ...
            const purchases = purchasesSnap.docs.map((d) => {
                ...
                return {
                    ...
                    purchasedAt: p.createdAt?.toDate?.()?.toISOString?.() || null,
                    accessUrl: p.accessUrl || (p.accessToken ? `${base}/access.html?token=${p.accessToken}` : null),
                    coverUrl: p.productId ? `${apiBase}/getProductCover?productId=${encodeURIComponent(String(p.productId))}` : null,
                };
            });

            await db.collection("buyerLibraryLogs").add({
                emailHash,
            ...
            res2.status(200).json({ success: true, email: emailNorm || null, purchases });
        } catch (e) {
            console.error(FN, e);
            await logSystemError(FN, e, { body: req2.body });
            res2.status(e.httpStatus || 500).json({ success: false, error: e.message || "Internal error", code: e.code || "internal" });
...
exports.accessContent = onRequest({ invoker: "public" }, async (req, res) => {
    corsMiddleware(req, res, async (req2, res2) => {
        ...
        try {
            ...
            if (!token) throw createHttpError(400, "invalid_argument", "Missing access token.");

            const ordersSnap = await db.collection("orders").where("accessToken", "==", String(token)).limit(1).get();
            if (ordersSnap.empty) throw createHttpError(404, "not_found", "Invalid access token.");
            ...
            if (order.deviceFingerprintHash) {
                const orderFpHash = String(order.deviceFingerprintHash);
                let allowed = fpHash && fpHash === orderFpHash;

                if (!allowed && fpHash) {
                    const mapSnap = await db.collection("deviceToBuyer").doc(fpHash).get();
                    const emailHash = sha256Hex(normalizeEmail(order.buyerEmail || ""));
                    if (mapSnap.exists && String(mapSnap.data()?.emailHash || "") === emailHash) {
                        allowed = true;
                    }
                }
                ...
                if (!allowed) {
                    await db.collection("accessAttempts").add({
                        ...
                        orderDeviceFingerprintHash: orderFpHash,
                        attemptDeviceFingerprintHash: fpHash,
                        createdAt: nowServerTimestamp(),
                        allowed: false,
                    });
                    throw createHttpError(403, "permission_denied", "You can only access content from your verified device.");
                }
            } else {
                const deviceInfo = extractDeviceInfo(userAgent);
                const originalDevice = order.deviceInfo || {};
                const originalIsUnknown = isUnknownDeviceInfo(originalDevice);
                ...
                if (!isSameDevice) {
                    ...
                    throw createHttpError(403, "permission_denied", "You can only access content from purchase device.");
                }

                if (originalIsUnknown) {
                    await orderDoc.ref.set({ deviceInfo, updatedAt: nowServerTimestamp() }, { merge: true });
                }
            }

            // ✅ Product can be active or archived (buyers keep access)
            ...
            for (const a of Array.isArray(product.assets) ? product.assets : []) {
                ...
                files.push({
                    ...
                    downloadUrl: `${apiBase}/downloadProductAsset?token=${encodeURIComponent(String(token))}&productId=${encodeURIComponent(
                        String(order.productId)
                    )}&index=${encodeURIComponent(String(a.index))}`,
                });
            }

            const coverUrl = product.coverPath ? `${apiBase}/getProductCover?productId=${encodeURIComponent(String(order.productId))}` : null;
            ...
            if (order.status === "completed") {
                await db.runTransaction(async (tx) => {
                    ...
                    if (o.status === "completed") {
                        tx.update(orderDoc.ref, { status: "processed", processedAt: nowServerTimestamp() });
                        didProcess = true;
            ...
            if (didProcess && sellerUid) await statsInc(sellerUid, { processedCount: 1 });
...
exports.downloadProductAsset = onRequest({ invoker: "public" }, async (req, res) => {
    corsMiddleware(req, res, async (req2, res2) => {
        ...
        try {
            const token = String(req2.query.token || "");
            const productId = String(req2.query.productId || "");
            const index = Number(req2.query.index || 0);
            const userAgent = String(req2.get("user-agent") || req2.query.userAgent || "");
            ...
            if (!productId) throw createHttpError(400, "invalid_argument", "Missing productId.");
            if (!Number.isFinite(index) || index < 1) throw createHttpError(400, "invalid_argument", "Invalid index.");
            ...
            const orderDoc = ordersSnap.docs[0];
            const order = orderDoc.data();
            if (String(order.productId) !== productId) throw createHttpError(403, "permission_denied", "Token does not match product.");
            ...
            if (order.deviceFingerprintHash) {
                ...
                if (!allowed) throw createHttpError(403, "permission_denied", "You can only access content from your verified device.");
            } else {
                const deviceInfo = extractDeviceInfo(userAgent);
                const originalDevice = order.deviceInfo || {};
                const originalIsUnknown = isUnknownDeviceInfo(originalDevice);
                const isSameDevice =
                    originalIsUnknown ||
                    (deviceInfo.browser === (originalDevice.browser || "") && deviceInfo.os === (originalDevice.os || ""));
                if (!isSameDevice) throw createHttpError(403, "permission_denied", "You can only access content from purchase device.");
                if (originalIsUnknown) await orderDoc.ref.set({ deviceInfo, updatedAt: nowServerTimestamp() }, { merge: true });
            }

            // ✅ Active or archived product lookup
            const loaded = await loadProductById(productId);
            if (!loaded) throw createHttpError(404, "not_found", "Product not found.");
            const product = loaded.data || {};

            const asset = (Array.isArray(product.assets) ? product.assets : []).find((a) => Number(a.index) === index);
            if (!asset?.filePath) throw createHttpError(404, "not_found", "File not found.");
            ...
            res2.setHeader("Content-Disposition", `${inlineOk ? "inline" : "attachment"}; filename="${filename.replace(/"/g, "")}"`);
            ...
            if (Number.isFinite(size) && size > 0) res2.setHeader("Content-Length", String(size));
